...
状态预计分钟: 数字，无法判断则 0
状态不便看手机: 是|否
状态底座: "一句话状态底座，格式为：用户此刻/刚才的状态 + 关系氛围/下一轮注意点；只写确定信息，无则写 无"
关系氛围: "一句话概括当前关系氛围或用户对回复的感受，如轻松、暧昧、认真质疑、不舒服，无则写 无"
回复注意: "下一轮回复最应该注意的一句话，如不要贬低用户选择、不要强行调侃、不要问在干嘛，无则写 无"
</hermes_classify>

规则（全部用中文）:
- 状态底座不是日志摘要，也不是系统视角说明；它是给下一轮回复使用的轻量上下文。
- 状态底座必须由你根据上下文自然生成，不要套模板，不要做关键词替换。
- 状态底座只允许使用“用户”和“你”来指代双方；“你”表示当前陪伴角色自己。不要写“助手、AI、机器人、模型、角色”等系统身份词。
- 时间表达要贴合当前会话：如果是刚刚发生的一轮，用“刚才”或直接省略时间；只有确实跨会话/跨较长时间时，才写具体时间差，不要泛泛写“之前”。
- 状态底座要像一句给自己看的聊天便签，例如：用户深夜打招呼，氛围轻松；下一轮别追问太多。
...
- 如果用户当前有短期状态或正在进行的事情，但你认为不需要普通主动回访，仍然要认真填写“状态预计分钟”；系统会在超过预计时间后做一次轻量兜底询问。
- 兜底询问不是频繁回访：只有短期状态明确且状态预计分钟大于 0 时才会使用。
